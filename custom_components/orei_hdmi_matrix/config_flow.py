...
class OptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_init(
        ...
        for i in range(1, NUM_INPUTS + 1):
            ...
            input_fields[vol.Required(name_key, default=default_name, description=f"Input {i} Name")] = str
            
            # Input enabled
            enabled_key = f"input_{i}_enabled"
            default_enabled = inputs.get(str(i), {}).get(CONF_INPUT_ENABLED, True)
            input_fields[vol.Required(enabled_key, default=default_enabled, description=f"Enable Input {i}")] = bool
